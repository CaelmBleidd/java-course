...
public class Implementor implements Impler, JarImpler {
    ...
    public void implement(Class<?> aClass, Path path) throws ImplerException {
        ...
        try (BufferedWriter code = createFile(aClass, path)) {
            try {
                code.write(toUnicode(genHead(aClass) + NEWLINE));
                if (!aClass.isInterface()) {
                    ...
                    code.write(toUnicode(stringOfConstructors));
                }
                code.write(toUnicode(genAbstractMethods(aClass)));
                code.write(toUnicode(getTail(aClass)));
            } catch (IOException e) {
